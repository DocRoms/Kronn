...
AI context last reviewed: **2026-04-20** (v0.5.0 released — plugin kind (MCP/API/hybrid) + first API plugin Chartbeat, worktree test-mode flow, decoder-loop detection + thinking-tag strip, Claude Code prompt-via-stdin, workflow-run cancel rescue for orphans, QP "Analyse de ticket Jira" hardened after real-run audit, vendor-neutral tests).
