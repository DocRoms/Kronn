...
- **Tests**: Vitest 4 (`npm test`, 504 tests, 38 suites). Use @testing-library/react for component tests. Wrap state-triggering calls in `act()`. For api mocking, prefer the shared `src/test/apiMock.ts` factory over inline `vi.mock` factories — see `ai/testing-quality.md`.
