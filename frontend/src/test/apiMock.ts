...
export function buildApiMock(overrides: PartialDeep<DefaultMock> = {}): DefaultMock {
  const base: DefaultMock = {
    ...
    config: {
      ...
      getRecoveryStatus: resolve({ configured: false }),
      setRecovery: resolve({ recovery_code: 'KRECOV1.mock.mock' }),
      restoreRecovery: resolve(undefined),
    ...
    projects: {
      ...
      cloneAndRemap: resolve({ project_id: '', new_path: '' }),
      ...
      discSources: resolve([]),
      // KT-74 — same default for portable-import provenance: no import, no badge.
      discImports: resolve([]),
