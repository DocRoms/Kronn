...
export function CollectionShell<TItem>({
    ...
    <aside className={`collection-shell-sidebar ${sidebarClassName}`.trim()} data-mobile={isMobile} aria-label={ariaLabel} onKeyDown={onSidebarKeyDown}>
      ...
      {slots.renderSearch ? slots.renderSearch(searchContext) : <header className="collection-shell-header">
        <div className="collection-shell-search">
          ...
          <input id={searchInputId} className="collection-shell-search-input" value={persistence.query} onChange={event => persistence.onQueryChange(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') onSearchSubmit?.(); }} aria-label={labels.search} aria-keyshortcuts="/" placeholder={labels.search} />
        ...
        {(slots.sidebarHeaderEnd || (title == null && isMobile)) && <div className="collection-shell-search-actions">
          ...
          {title == null && isMobile && <button type="button" className="collection-shell-icon" onClick={collapseSidebar} aria-label={labels.closeCollection}><X size={17} /></button>}
        </div>}
      </header>}
  ...
  return <section className="collection-shell" data-mobile={isMobile}>
    ...
    <div className="collection-shell-detail">
      {!sidebarOpen && isMobile && <button type="button" className="collection-shell-open" onClick={openSidebar} aria-label={labels.openCollection} autoFocus={focusRailOnMount}><Menu size={18} /></button>}
