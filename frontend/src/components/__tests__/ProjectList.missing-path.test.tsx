...
const dockerRunning = vi.hoisted(() => vi.fn().mockResolvedValue({ project_ids: [], checked_at: '2026-08-30T08:00:00Z' }));
vi.mock('../../lib/api', () => ({ projects: { delete: deleteProject, dockerRunning } }));
...
function renderList(projects: Project[], loading = false) {
  return render(
    <ProjectList
      projects={projects}
      loading={loading}
...
describe('ProjectList — missing-path banner', () => {
  beforeEach(() => {
    ...
    dockerRunning.mockClear();
    dockerRunning.mockResolvedValue({ project_ids: [], checked_at: '2026-08-30T08:00:00Z' });
  ...
  it('renders the project empty state through the real collection adapter', () => {
    renderList([]);
    expect(screen.getByText('projects.emptyHint')).toBeInTheDocument();
  });

  it('keeps the loading state inside the mounted master/detail shell', () => {
    const { container } = renderList([], true);

    const shell = container.querySelector('.project-shell');
    expect(shell).not.toBeNull();
    expect(shell?.querySelector('.collection-shell-sidebar .project-sidebar-loading')).not.toBeNull();
    expect(shell?.querySelector('.collection-shell-detail > .project-detail-loading'))
      .toHaveTextContent('projects.loading');
    expect(container.querySelector('.dash-loading-bar')).toBeNull();
    expect(screen.queryByText('projects.emptyHint')).toBeNull();
  });

  it('covers the narrow rail, empty list, selection, and open menu on the real project list', () => {
    ...
    expect(screen.getByRole('checkbox', { name: /Alpha.*collection\.selectItem/ })).toBeChecked();
  });
  ...
  it('marks running Docker projects and filters the list to them', async () => {
    dockerRunning.mockResolvedValueOnce({
      project_ids: ['p2'],
    ...
    expect((await screen.findAllByLabelText('projects.master.dockerUp'))[0]).toHaveTextContent('UP');
    fireEvent.click(screen.getByRole('button', { name: 'projects.master.filter' }));
    fireEvent.click(screen.getByRole('button', { name: 'projects.master.filter.dockerRunning' }));

    const allProjects = screen.getByTestId('project-section-all');
    expect(within(allProjects).queryByTestId('project-list-item-p1')).not.toBeInTheDocument();
    expect(within(allProjects).getByTestId('project-list-item-p2')).toBeInTheDocument();
  });
  ...
  it('keeps restored favorites while the project request is still loading', () => {
    localStorage.setItem('kronn:collection-favorites:projects', JSON.stringify(['p1']));
    const props = {
      discussions: [], discussionsByProject: {}, driftByProject: {}, agents: [], allSkills: [], mcpConfigs: [], workflows: [],
      configLanguage: 'fr', toast: noop, onNavigate: noop, onSetDiscPrefill: noop, onAutoRunDiscussion: noop,
      onOpenDiscussion: noop, onRefetch: noop, onRefetchDiscussions: noop, onRefetchSkills: noop, onRefetchDrift: noop,
    ...
    const { rerender } = render(<ProjectList {...props} projects={[]} favoritesReady={false} />);

    expect(JSON.parse(localStorage.getItem('kronn:collection-favorites:projects') ?? '[]')).toEqual(['p1']);

    rerender(<ProjectList {...props} projects={[proj('p1', 'Alpha', '/repos/alpha', true)]} favoritesReady />);
    expect(within(screen.getByTestId('project-section-favorites')).getByText('Alpha')).toBeInTheDocument();
  });
