...
| TD-20260724-workflow-agent-fallback | Agent workflow steps bind one agent and fail with it when the provider is rate-limited or unavailable; there is no explicit, capability-aware fallback chain or failover audit trail. Detail: `docs/tech-debt/TD-20260724-workflow-agent-fallback.md`. | Backend / Workflows / Agents | Medium |
