...
| TD-20260808-http-agents-no-tool-calling | **Core shipped 2026-08-09; workflow parity added in 0.9.7**: HTTP agents (Ollama, LiteLLM) call a bounded native `tools`/`tool_calls` catalogue in discussions and Workflow Agent steps. Workflow execution is project-scoped, excludes Planning mutations, retains secret-free call receipts and explicitly reports when no call was recorded. **Residual**: no per-model capability probe, no dedicated tool-exposure policy beyond the existing project/API scope. Detail: `docs/tech-debt/TD-20260808-http-agents-no-tool-calling.md`. | Backend / Agents | Medium |
| TD-20260807-litellm-integration-missing | LiteLLM integration **shipped 2026-08-09**: `AgentType::LiteLlm`, install via Kronn's existing button (`uv tool install 'litellm[proxy]' --with 'fastapi<0.140'` — the pin is required, upstream's range allows a fastapi that breaks the proxy), `/api/lite-llm/{health,models,test}`, and a two-step settings card (declare endpoint + key → test → per-tier model pickers) since nothing about a proxy is auto-detectable. Transport shared with Ollama behind `ChatCodec`; bounded native tools now work in discussions and Workflow Agent steps. **Residual**: per-model tool capability detection remains tracked in `TD-20260808-http-agents-no-tool-calling`. Detail: `docs/tech-debt/TD-20260807-litellm-integration-missing.md`. | Backend / Agents | Medium |
