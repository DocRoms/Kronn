...
Dashboard pages (current / planned). Since the React Router v8 migration every
page — and every selectable resource — is URL-addressable, so views survive
refresh, browser back/forward works, and deep-links are shareable:
`/projects/:id?` · `/discussions/:id?` · `/planning/:taskId?` ·
`/plugins/:configId?` · `/workflows` (+ `/workflows/:id/runs/:runId`,
`/workflows/qp/:qpId?`, `/workflows/qa/:qaId?`) · `/config`. The route table is
`frontend/src/router.tsx`; navigation goes through `useKronnNavigate()`. The
CLI's legacy `#project-<id>` hash link redirects to `/projects/:id`.
...
Note: the old "Agents" page has been merged into Config. Nav order: Projets (`/projects`) → Discussions (`/discussions`) → Planification (`/planning`) → Plugins (`/plugins`) → Workflows (`/workflows`) → Config (`/config`). **"?" button** in nav replays the guided tour.
