...
| i18n | Custom lazy-loaded system (fr/en/es/zh), no external lib. The active UI locale is cached in localStorage and persisted as `ui_language`; it is independent from the discussion/output `language`. Every UI key must exist in all four locale modules. |
...
| Agents | Claude Code CLI, OpenAI Codex CLI, Vibe (Mistral), Gemini CLI (Google), Kiro (Amazon), **Ollama** (local HTTP) and **LiteLLM** (OpenAI-compatible proxy). LiteLLM's two-step settings card requires endpoint/key validation before exposing proxy-declared models; after validation, temporary endpoint/VPN failures keep the saved endpoint and assigned tiers visible instead of returning to the credential form. `[src: file: frontend/src/components/settings/LiteLlmCard.tsx:68-94]` `[src: file: frontend/src/components/settings/LiteLlmCard.tsx:337-385]` The endpoint lives in `AgentConfig.base_url`, key in the encrypted token store under `litellm`. Every discussion, orchestration and workflow call site that can start LiteLLM must forward that saved URL through `AgentStartConfig.lite_llm_base_url`; omitting it silently selects the localhost fallback. `[src: file: backend/src/agents/runner.rs:435-501]` `[src: file: backend/src/agents/runner.rs:1689-1704]` `/v1/models` is only a declared catalogue, not proof that the upstream project/region can invoke an alias. Kronn remembers non-transient discussion model-start failures per endpoint/model. Retry checks the live catalogue before attempting a completion: a removed alias is marked as such without invoking it, while a catalogued alias is cleared only after a successful minimal completion. A remembered diagnostic may also be manually removed for the active endpoint without probing the model; a later runtime failure records it again. `[src: file: backend/src/db/lite_llm_model_failures.rs:17-70]` `[src: file: backend/src/api/lite_llm.rs:223-245]` `[src: file: backend/src/api/lite_llm.rs:389-465]` `[src: file: frontend/src/components/settings/LiteLlmCard.tsx:243-255]` The HTTP wire format sits behind `ChatCodec`: `OllamaCodec` (NDJSON) and `OpenAiCodec` (SSE + usage frame). Ollama and LiteLLM system context identifies the concrete runtime model/route and maps the provider alias to self, preventing a local model from copying another participant's Claude identity out of the transcript. `[src: file: backend/src/agents/runner.rs:539-557]` Since 0.9.4, both HTTP agents receive native `tools`/`tool_calls` declarations from `backend/src/agents/tools.rs`; `backend/src/api/agent_tools.rs` executes the bounded in-process loop. CLI agents continue to use the MCP bridge. Planned: OpenCode, DeepSeek. |
...
Discussion multi-model invariant (0.9.4): active placeholders are keyed by durable dispatch + source User turn, and late native replies render inside their originating turn without rewriting append-only message order. `[src: file: backend/src/models/discussions.rs:12-27]` `[src: file: frontend/src/lib/messageTargets.ts:159-261]`

Discussion transport invariant (0.9.5): the WebSocket heartbeat has an acknowledgement deadline and reconnects with bounded exponential backoff; every reconnect reloads the discussion list, active room and contact presence. A discussion shows the reconnecting state explicitly. A submitted message remains a durable local draft until the SSE stream emits `accepted`, which the backend sends only after persistence; any pre-receipt failure removes the optimistic row and restores the draft. `[src: file: frontend/src/hooks/useWebSocket.ts:7-178]` `[src: file: frontend/src/pages/DiscussionsPage.tsx:1173-1192]` `[src: file: frontend/src/pages/DiscussionsPage.tsx:1949-2068]` `[src: file: frontend/src/pages/DiscussionsPage.tsx:3041-3048]` `[src: file: frontend/src/components/ChatInput.tsx:362-417]`

Composer and model-selection invariant (0.9.4): new discussions, normal chat and workflow/Quick Prompt agent prompts reuse `MarkdownEditor` for edit/preview tabs, contextual Markdown and emoji help, and insertable examples. Agent/model changes reuse `AgentSwitchPicker`; mention targets persist their own reasoning tier instead of inheriting the discussion agent's choice. `[src: file: frontend/src/components/MarkdownComposerTools.tsx:20-317]` `[src: file: frontend/src/components/AgentSwitchPicker.tsx:13-256]` `[src: file: frontend/src/components/NewDiscussionForm.tsx:744-780]`

Workflow-editor invariant (0.9.4): the wizard is a fixed workspace whose stage outline and footer remain visible while the editor pane scrolls. Editing an existing workflow permits direct stage navigation; the Steps stage also lists and scrolls to each individual step. Cancel is always available and Save is available from every stage during editing. `[src: file: frontend/src/components/workflows/WorkflowWizard.tsx:964-1045]` `[src: file: frontend/src/components/workflows/WorkflowWizard.tsx:4117-4163]` `[src: file: frontend/src/pages/WorkflowsPage.css:1066-1230]`
...
**Written ≠ loaded.** An agent can reject a valid config for reasons Kronn cannot see, and the failure is silent on both sides. Vibe is the case currently probed: it resolves workspace trust against the `.vibe` directory itself and honours the *nearest* decision, so an untrusted `.vibe` overrides a trusted repository root and Vibe then loads **zero** MCP servers from a file that lists them. `backend/src/core/vibe_trust.rs` mirrors that resolution read-only; `warn_if_vibe_trust_blocks` (in `mcp_scanner.rs`) warns at write time on `kronn::host_sync`, `detect_runtime_warning` surfaces `vibe.project_config_untrusted` on the agent card, and `kronn doctor` lists every blocked config. **Kronn never writes to `trusted_folders.toml`** — re-granting trust is the user's security decision. The equivalent probe for Codex/Gemini/Kiro/Copilot does not exist yet: see `TD-20260807-agent-config-load-unverified`.
...
**Current review: 2026-08-11 (0.9.5 release-quality pass).** Current invariants live in the focused sections above; release history belongs in `CHANGELOG.md` and `docs/releases/`.
