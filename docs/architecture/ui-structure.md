...
`[src: file: backend/src/db/discussion_actions.rs]`
`[src: file: backend/src/api/discussion_actions.rs]`
`[src: file: frontend/src/components/DiscussionActionCard.tsx]`

**Right-pane header surface:** master/detail pages use
`.collection-detail-header` for the shared subtle accent gradient and bottom
separator; each page keeps ownership of its header layout and spacing. The
content pane sits inside the full-bleed page shell with the shared compact inset,
border and radius; Discussions and Pages apply that frame directly to their
long-lived right pane so their internal scroll areas remain stable.
`[src: file: frontend/src/pages/DiscussionsPage.css:1900-1921]`
`[src: file: frontend/src/pages/PagesPage.css:76-99]`
Automation has no additional page-title band: the selected workflow, Quick
...
**Run with an agent**. The launch dialog selects the worker/model/profile,
workspace, target branch, validation commands, review limit, fallback agents and
opt-in automatic continuation. The execution card follows the durable
`TaskExecution` lineage, opens the child discussion/worktree, reports model,
duration/tokens/cost and exposes only state-valid stop, reassign and review
actions. The parent remains the review and guarded-integration authority. See
[`adr-002-orchestration-multi-agent.md`](../design/adr-002-orchestration-multi-agent.md)
and the [operator guide](../guides/task-orchestration.md).
...
`[src: file: frontend/src/components/ProjectCard.tsx:1335-1350]`

The project Docker tab is intentionally Compose-only. It renders a useful empty
state when no root Compose file exists and an actionable availability state when
the Docker CLI, Compose plugin or daemon cannot be reached. Lifecycle buttons
are disabled while an operation is in flight and the returned status snapshot
replaces the previous service list after every action. Its configuration action
deep-links to the detected Compose file in the project's Code source explorer.
`[src: file: frontend/src/components/ProjectDockerPanel.tsx]`
`[src: file: frontend/src/components/SourceCodeViewer.tsx]`

**Discussion-agent failure recovery (0.9.7):** native-agent startup/model
failures are persisted as attributed System messages instead of leaving a
silent pending target. LiteLLM/Ollama connectivity failures and HTTP
model/catalogue errors share one compact diagnostic card while preserving the
exact technical detail. `POST /api/discussions/:id/agent-dispatches/retry`
creates an idempotent retry for
only the failed dispatch, anchored to its original trigger message; successful
sibling targets are never replayed. Legacy `[kronn:model-error]` messages remain
readable while new events use `[kronn:agent-error]`.

**Per-reply cancellation:** every durable pending/running agent response is
rendered in its source turn with an explicit queued/running state and a compact
Stop action. The action addresses the dispatch id, not the discussion or the
agent type, so cancelling an older response preserves later responses from the
same agent. `[src: file: frontend/src/pages/DiscussionsPage.tsx:80-189]`
`[src: file: backend/src/api/discussions/messaging.rs:1137-1219]`

**Message deletion and authorship:** deleting an individual discussion message
removes its original payload, message-scoped attachments and routing metadata,
but deliberately retains the row as a compact tombstone. The tombstone keeps
the timeline continuous for humans and enters future agent context with an
explicit instruction not to reconstruct the missing content. A trigger message
cannot be deleted while one of its dispatches is pending or running. Automated
follow-ups authored by the `Orchestrateur` persona use a dedicated left-aligned
purple treatment and explicit “automated follow-up” attribution, distinct from
human user bubbles. Their card spans the full message column like a discussion
note. The first meaningful line stays visible as a compact summary while the
full operational payload is collapsed behind “Show details” by default; that
...
row. The full “Routing requested” label remains available to assistive
technology and as a pointer tooltip. `[src: file: frontend/src/components/MessageBubble.tsx]`

**Accessible agent identity colours:** provider brand colours remain available
for dots, borders and tinted surfaces. Text labels use `agentTextColor`; NVIDIA
therefore keeps its `#76B900` brand accent while using a WCAG-AA companion green
adapted to light and dark themes in aliases, message attribution, comparisons,
workflow summaries and the shared agent selector.
`[src: file: frontend/src/lib/constants.ts]`
`[src: file: frontend/src/styles/tokens.css]`

**Quick Prompt Compare details:** a completed or live Compare opens a dedicated
multi-column workspace whose columns can be reordered without losing their
discussion identity. Its Details panel ranks by weighted quality, AI quality,
human quality, duration or tokens; missing metrics remain unavailable rather
than becoming zero. Human and AI stars are durable and independent. The human
rating is directly editable, while a selected agent+tier can run Kronn's
versioned blind rubric over anonymous answers. AI verdicts persist confidence,
positive/negative points, contract violations and judge provenance (model,
duration and tokens). Weighted quality exposes its human/AI weights and
renormalizes only over present ratings instead of hiding missing evidence.
`[src: file: frontend/src/components/BatchCompareDetailsPanel.tsx]`
`[src: file: backend/src/api/compare.rs]`
`[src: file: backend/src/db/sql/130_batch_compare_evaluations.sql]`
...
Workflow bundle v2 includes Quick Prompts referenced by Agent, BatchQuickPrompt
and chained batch steps; Quick APIs referenced directly or by CollectApiData;
static Page HTML/dataset contracts; and transitive sub-workflows. Import mints
and remaps fresh ids. It strips per-user fields such as `gate_notify_url`, does
not export API credentials, and excludes retained Page observations so sharing
a workflow does not leak runtime data.
...
Doc utilisateur complète : [`docs/ticket-autopilot.md`](../ticket-autopilot.md).
...
**2. `quick_prompt_id` sur `StepType::Agent`** — symétrique. Helper `quick_prompt_hydrate::hydrate_step_from_quick_prompt` injecte `prompt_template`, `tier`, et `skill_ids` du QP au run-time si le step les a vides. Les `{{var}}` du QP sont résolus avec le `TemplateContext` du workflow (launch variables / state / previous_step / steps.X). Le mapping optionnel `quick_prompt_variables` relie un nom portable du QP à une expression dynamique du workflow, par exemple `{ "production_errors": "{{steps.shape.data.toppages}}" }`. Wizard expose un picker dans la step Agent card.
