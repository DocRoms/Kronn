...
**host_sync (HostSyncMode)** — 0.6.0 — `mcp_configs.host_sync` column controlling whether a Kronn-managed MCP is also written into the user's local CLI config files (`~/.claude.json`, `~/.gemini/settings.json`, `~/.codex/config.toml`, `~/.copilot/mcp-config.json`). Values: `None` (Kronn-only) | `GlobalOnly` (canonical "synced to host") | `MirrorAll` (legacy — collapsed to `is_global=true + host_sync='GlobalOnly'` by migration 038, still accepted as a defensive read fallback).

**`_kronn` marker** — 0.6.0 — JSON object Kronn embeds inside every entry it writes to a host CLI config file: `{ "managed": true, "config_id": "<uuid>" }`. Lets Kronn distinguish entries it owns (and may rewrite/delete) from entries the user added manually (preserved verbatim across syncs). The orphan-cleanup pass walks the whole tree and removes entries whose `config_id` no longer exists in `mcp_configs`. Survives reformatting (more reliable than the hash fallback for ownership detection).

**Claude scope-aware routing** — 0.6.0 — `sync_claude_global_config` chooses where to write each Kronn-managed entry based on the config's Kronn scope: `is_global=true` → top-level `mcpServers` (Claude scope `user`); `is_global=false + project_ids=[…]` → `projects[<host-path>].mcpServers` for each project (Claude scope `local`). Other CLIs (Gemini, Codex, Copilot) have no native per-project scope in their config files, so they're always top-level — surfaced as an asymmetry hint in the UI's `HostSyncPreview`.

**HostScope (host MCP discovery)** — 0.6.0 — Enum returned by `host_mcp_discovery::scan_all_host_mcps`: `ClaudeUser` (~/.claude.json top-level), `ClaudeLocal { project_path }` (~/.claude.json projects[…]), `Gemini`, `Codex`, `Copilot`. Used to disambiguate the same MCP name in multiple Claude scopes during inbound discovery + adopt.
...
**OpenCode** — SST's CLI coding agent (`opencode` command via `npm install -g opencode-ai`). Runs as a native ACP runtime (`opencode acp`, production default — no adapter, no opt-in toggle needed) rather than a direct-CLI spawn; see `docs/operations/acp-adapters.md` and `docs/design/adr-003-acp-control-plane.md`. Auth: `opencode auth login` (credentials at `~/.local/share/opencode/auth.json`) is one path among several the CLI accepts — environment credentials, its own provider config, and local/no-auth providers all work too, so Kronn treats a missing/invalid auth file as unknown rather than a confirmed "not ready" (`backend/src/agents/mod.rs::opencode_auth_ready`). Color: `#7c3aed`. Models/tiers flow through the KT-531 dynamic catalog (`docs/architecture/model-catalogs.md`) — never a hardcoded model list, including for **OpenCode Zen** (SST's hosted model gateway, optional provider inside OpenCode's own config, models namespaced `opencode/<model-id>`).
...
**RTK (Rust Token Killer)** — External Rust tool (`github.com/rtk-docs/rtk`) that hooks into AI agent CLIs and compresses shell-command outputs (git, cargo, ls, test runners) before they reach the LLM. ~89 % compression observed. Kronn integrates RTK as the "Mode économique" section in Settings > Agents — see [`docs/architecture/rtk-integration.md`](architecture/rtk-integration.md) for the full contract (detection paths per agent, activation flags, savings parser). Kronn does NOT wrap agent shell calls itself; RTK owns the hook format, Kronn owns the detection + activation UX + savings readout.
