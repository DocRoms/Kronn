...
- **Why we can't fix now (constraint)**: the product shape is validated, but
  this cross-cutting feature is intentionally deferred until the dedicated
  0.9.1 implementation cycle, immediately after the 0.9.0 release.
  `[src: user: 2026-07-24: release roadmap renumbering]`
...
- **Next step (0.9.1)**: turn the design's implementation slices into tickets
  and begin with the domain schema plus compact read API.
