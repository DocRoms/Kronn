...
### `docs/checksums.json` — generated during audit
...
1. Read `docs/checksums.json` from the project directory
2. For each mapping, recompute SHA256 of each source file
3. Compare with stored checksums
4. Return: `{ stale_sections: ["docs/coding-rules.md", "docs/architecture/overview.md"], details: [...] }`
...
Accepts `{ steps: [4, 8] }`. Runs only the specified audit steps with the same prompts. Updates `docs/checksums.json` with new hashes. Much cheaper than a full 10-step audit.
...
|------|----------------|---------------------|
| 1 | docs/AGENTS.md | README.md, package.json, Cargo.toml, docker-compose.yml, Makefile |
| 2 | docs/glossary.md | (derived from other steps, no direct sources) |
| 3 | docs/repo-map.md | directory structure (hash of `find . -type f` output) |
| 4 | docs/coding-rules.md | package.json, Cargo.toml, tsconfig.json, .eslintrc*, rustfmt.toml |
| 5 | docs/testing-quality.md | package.json, Cargo.toml, .github/workflows/*, vitest.config.*, jest.config.* |
| 6 | docs/operations/debug-operations.md | docker-compose.yml, Makefile, Dockerfile* |
| 7 | docs/operations/mcp-servers.md | .mcp.json |
| 8 | docs/architecture/overview.md | docker-compose.yml, main entrypoints (src/main.*, src/lib.*, src/App.*) |
| 9 | docs/inconsistencies-tech-debt.md | (full scan — hash all source files, or use git diff since audit date) |
