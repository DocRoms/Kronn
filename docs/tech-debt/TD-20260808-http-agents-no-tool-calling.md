...
- **Residual**:
  ...
  - **The catalogue grew past the original five** (`mcp_list`, `api_endpoints`, `qa_list`, `qa_run`, `api_call`) with the workspace, planning, orchestration and — in 0.13.0 — media tools. Every entry costs context on a small model and a long list degrades selection, so each widening followed a demonstrated gap rather than completeness: the media pair was added because HTTP agents were the only half of the fleet that could not invoke a feature `kronn_intro` announces. Whether the assembled catalogue is now too long for a 4 B model has not been re-measured since the original projections — that measurement, not a count, is what should gate the next addition.
