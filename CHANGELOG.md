...
- Frontend : 1260 → **1333** vitest (+73) — `AuditRecapPanel.test.tsx` (+6), `SubAuditModal.test.tsx` (+7), `DiscussionSidebar.sourceBadge.test.tsx` (+5), `BriefingForm.test.tsx` (+5), `QuickPromptForm.bindings.test.tsx` (+5), `qp-improver-signal.test.ts` (+10), `MessageBubble.seedToggle.test.tsx` (+8), `QPHistoryDrawer.test.tsx` (+17), `WorkflowsPage.requiredVars.test.tsx` (+8) + petits ajouts (CTA / signals).
