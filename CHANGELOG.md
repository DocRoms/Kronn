...
**Désagentify + push→pull migration — réduction massive du token cost sur les surfaces non-audit.**
...
- **Sub-audits Database + ApiDesign prompts étoffés** (`api/audit/mod.rs::DATABASE_STEPS` + `API_DESIGN_STEPS` #287 partiel) — pre-0.8.4 ces 2 sub-audits étaient des placeholders 0.8.2 ("placeholder body, content lands in S2.D4-5"). Maintenant DATABASE_STEPS couvre : schema + migrations safety / indexes + perf / ORM + N+1 / data integrity ; API_DESIGN_STEPS couvre : contract consistency / versioning + evolution / pagination + list responses / authn + authz + rate limiting / doc drift. Tous deux suivent le même schema TD detail-file + anti-repetition + marker discipline que la Full audit Step 9. **Restent en 0.8.5** : sub-audit UI selector (Audit global vs Audit ciblé + tooltip explicatif) + nouveau kind `Rgaa` (norme française accessibility, sous-cas de A11y).
...
- Backend : 2043 → **2083** (+40) — linked_repos push→pull (+5) + audit_run_steps recap (+5) + RGAA kind (+1) + cross-agent memory DB helpers (+9) + 6 API integration tests for `disc_*` routes (+6) + ts-rs / shape pinning (+14)
- Frontend : 1260 → **1278** (+18) — `AuditRecapPanel.test.tsx` (+6), `SubAuditModal.test.tsx` (+7), `DiscussionSidebar.sourceBadge.test.tsx` (+5). Briefing form non testé unitairement — couvert par les tests Playwright en fin de release.

### Deferred to 0.8.5

— rien. 0.8.4 ship tout ce qui était prévu (#287, #294, #295, #285, #298) + #284 UX polish.
