...
- Backend : **2197 lib tests** (+17 net depuis 0.8.5) — agent_api broker (+9), validate_required_fields_per_type (+13), materialize_custom_server endpoints + back-compat (+4), update_custom_spec stitching + endpoints round-trip (+2). Quelques tests OAuth2 consolidés. ⚠ 2 tests `mcp_scanner_test::atomic_write_checked_*` flakes occasionnels sous parallel execution (race tempdir filesystem) — passent en isolation (6/6 quand isolés). Pre-existing flake, non-lié à 0.8.6, à fix avec tempdir-per-test isolation strict en 0.8.7.
- Frontend : **1393 vitest** — pas de variation nette ; nouveaux tests sur CustomApiAiHelper.endpoints (+6) + McpPage unified-edit + AuthKindPicker absorbés dans le pool existant.
- Python : 22 → **25 unittest** (+3 net) — `mcp_list.config_keys` + auth_managed inference + lowercase `${env.x}` substitution (broker live update vs spec lookup).
...
- **`[[project_cross_agent_collab_demo]]`** — invite-peer harness pour multi-Claude-Code sur disc partagée (~5h).
