...
- **Desktop: "Cannot connect to backend" on first launch** — startup health check was TCP-only (5s timeout). Now uses a real HTTP `GET /api/health` check with 15s timeout. Frontend auto-retries 5 times (2s interval) before showing the error screen
