...
Mesuré sur **EW-7247** (Africanews → Euronews multi-brand, migration
de phase 0, ~100k tokens en autopilot flat) : **-39 % de tokens**
(104,9k → 63,9k) et **-40 % de durée** vs un autopilot flat — en bonus
la détection d'une discrepancy ticket↔prod (channel_id) que l'agent a
remontée avec le fichier:ligne legacy en référence.
...
### Validated against EW-7247 (Africanews → Euronews multi-brand)
...
L'agent a détecté + remonté avec citation fichier une **discrepancy ticket↔prod sur `channel_id`** (ticket=2, prod=1 dans `parameters_euronews.yaml:2`) — bug que la prod aurait silencieusement absorbé dans une release sans contrôle.
