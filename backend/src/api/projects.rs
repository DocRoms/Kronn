...
pub async fn install_template(
    ...
    let install_result = tokio::task::spawn_blocking(move || -> Result<(), String> {
        ...
        if existing_docs.exists() {
            if let Err(e) = super::audit::check_ai_dir_permissions(&existing_docs) {
...
mod prompt_tests {
    ...
    // ─── ensure_agent_writable_subfolders (0.7.1 bootstrap pattern) ────
    ...
    fn ensure_subfolders_creates_three_folders_with_readme() {
        ...
        super::ensure_agent_writable_subfolders(&docs).unwrap();
    ...
    fn ensure_subfolders_noop_when_docs_dir_missing() {
        ...
        assert!(super::ensure_agent_writable_subfolders(&docs).is_ok());
        assert!(!docs.exists());
    }
}
