...
pub async fn update(
    ...
    match state
        ...
        .with_conn(move |conn| {
            ...
            let mut updated = crate::db::discussions::update_discussion(
                conn,
                &id,
                title.as_deref(),
                archived,
            ...
            if let Some(ref ids) = skill_ids {
                updated =
                    crate::db::discussions::update_discussion_skill_ids(conn, &id, ids)? || updated;
            }
            if let Some(ref ids) = profile_ids {
                updated = crate::db::discussions::update_discussion_profile_ids(conn, &id, ids)?
                    || updated;
            }
            if let Some(ref ids) = directive_ids {
                updated = crate::db::discussions::update_discussion_directive_ids(conn, &id, ids)?
                    || updated;
            }
            ...
            if let Some(strategy) = summary_strategy {
                updated = crate::db::discussions::update_discussion_summary_strategy(
                    conn, &id, strategy,
                )? || updated;
            }
        ...
        .await
    {
        ...
        Ok(false) => Json(ApiResponse::err(
            "Discussion not found or no fields to update",
        )),
        Err(e) => Json(ApiResponse::err(format!("DB error: {}", e))),
    }
}
