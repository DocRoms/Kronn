...
Otherwise this step is INCOMPLETE — it means you re-read the existing files instead of re-scanning the code.\n\n",
    ...
    block.push_str(
        "### Known debt (id — severity — title) — carry these in the index, don't duplicate\n",
    );
    for p in priors {
        let sev = if p.severity.is_empty() {
            "?"
        } else {
            &p.severity
        };
...
fn is_valid_td_id(token: &str) -> bool {
    let Some(rest) = token.strip_prefix("TD-") else {
        return false;
    };
...
pub fn render_report(
    ...
    out.push_str(&format!(
        "# Reconciliation report — {audit_date} {audit_kind}\n\n"
    ));
    out.push_str(
...
(Fixed / Stale / Missed / Uncertain) so nothing slips silently between audits.\n\n",
    ...
    out.push_str(&format!(
        "- **{}** re-emitted (priors reused — healthy: refreshed or carried over)\n",
        reused.len()
    ));
    out.push_str(&format!(
        "- **{total_candidates}** reconciliation candidates :\n"
    ));
    ...
    for label in &["Fixed", "Stale", "Missed", "Uncertain"] {
        let Some(list) = buckets.get(*label) else {
            continue;
        };
        if list.is_empty() {
            continue;
...
fn is_scaffolding(name: &str) -> bool {
    name == "README.md" || name == "TEMPLATE.md" || name == "_template.md" || name.starts_with('_')
}
...
pub fn parse_pointers(content: &str) -> Vec<Pointer> {
    ...
    for line in content.lines() {
        ...
        let Some(backtick_end) = rest[backtick_start + 1..].find('`') else {
            continue;
        };
...
mod tests {
    ...
    fn compute_delta_detects_unchanged_updated_deleted() {
        ...
        let by_id: HashMap<_, _> = deltas
            .iter()
            .map(|(s, d)| (s.id.clone(), d.clone()))
            .collect();
...
";
    ...
    fn check_signature_some_true_when_pointer_line_exists() {
        ...
        std::fs::write(&td_path, "## Where (pointers)\n- `src/foo.rs:2` — line2\n").unwrap();
...
"#;
        ...
        assert_eq!(
            ptrs.len(),
            3,
            "should pick exactly the 3 pointers under Where"
        );
        assert_eq!(ptrs[0].file, "src/foo.rs");
        assert_eq!(ptrs[0].line, Some(42));
        assert_eq!(ptrs[1].file, "src/bar.rs");
    ...
    fn parse_pointers_handles_line_range_takes_start() {
        ...
        assert_eq!(
            ptrs[0].line,
            Some(7),
            "ranges should yield the start line, not the end"
        );
    }
    ...
    fn parse_pointers_extracts_snippet_when_present() {
        ...
        assert!(ptrs[0]
            .snippet
            .as_ref()
            .is_some_and(|s| s.contains("doomed")));
    }
    ...
    fn parse_pointers_drops_snippet_when_too_short_or_too_long() {
        ...
        assert!(
            pshort[0].snippet.is_none(),
            "<8-char snippet must be discarded"
        );
        ...
        assert!(
            plong[0].snippet.is_none(),
            ">200-char snippet must be discarded"
        );
    }
...
"#;
    ...
    fn split_file_line_handles_trailing_colon_as_non_line() {
        ...
        assert_eq!(
            file, "src/foo.rs:",
            "trailing colon without digits keeps colon in path"
        );
        assert_eq!(line, None);
    }
    ...
    fn snapshot_returns_empty_for_dir_with_only_scaffolding() {
        ...
        assert!(
            snaps.is_empty(),
            "scaffolding files must not be in the snapshot"
        );
    }
...
";
        ...
        assert_eq!(
            ids.len(),
            2,
            "two distinct ids despite one being cited twice"
        );
    ...
    fn compute_delta_with_index_marks_relisted_prior_as_carried() {
        ...
        assert_eq!(
            deltas[0].1,
            DeltaKind::Carried,
            "re-listed verbatim prior must be Carried, not Unchanged"
        );
        ...
        assert_eq!(
            deltas[0].1,
            DeltaKind::Unchanged,
            "id absent from index → Unchanged candidate"
        );
    }
    ...
    fn docroms_scenario_relisted_priors_are_reused_not_missed() {
        ...
        assert!(
            !report.contains("## ⚠️ Missed"),
            "no finding should be Missed:\n{report}"
        );
        assert!(
            report.contains("**0** reconciliation candidates"),
            "report: {report}"
        );
    }
    ...
    fn digest_prior_tech_debt_skips_scaffolding_and_sorts_by_severity() {
        ...
        std::fs::create_dir_all(&td).unwrap();
        std::fs::write(
            td.join("TD-20260101-low.md"),
            "# Low thing\n**Severity**: Low\n",
        )
        .unwrap();
        std::fs::write(
            td.join("TD-20260101-crit.md"),
            "# Crit thing\n**Severity**: Critical\n",
        )
        .unwrap();
    ...
    fn render_known_debt_block_lists_priors_and_states_zero_new_is_failure() {
        ...
        assert!(
            block.contains("KEEP listing"),
            "priors must stay in the index = carried"
        );
        assert!(block.contains("Do NOT silently drop"));
        assert!(block.contains("duplicate"));
        assert!(
            !block.contains("do not re-emit these"),
            "old carried-hostile wording must be gone"
        );
