...
pub fn managed_targets(project_path: &Path, step_targets: &[String]) -> Result<Vec<PathBuf>, String> {
    ...
    match std::fs::symlink_metadata(&td_dir) {
        Ok(meta) if meta.file_type().is_symlink() => {
            return Err(format!("{}: tech-debt directory is a symlink — refusing to follow", td_dir.display()));
        }
        ...
        Ok(_) => {
            ...
            for entry in rd {
                let entry = entry.map_err(|e| format!("{}: directory entry failed: {e}", td_dir.display()))?;
                let p = entry.path();
...
fn refuse_symlink_chain(project_path: &Path, path: &Path) -> Result<(), String> {
    let relative = path.strip_prefix(project_path)
        .map_err(|_| format!("{} escapes project root {}", path.display(), project_path.display()))?;
    let mut cursor = project_path.to_path_buf();
    for component in relative.components() {
        if !matches!(component, Component::Normal(_)) {
            return Err(format!("{} contains an unsafe path component", path.display()));
        }
        cursor.push(component.as_os_str());
        match std::fs::symlink_metadata(&cursor) {
            Ok(meta) if meta.file_type().is_symlink() => {
                return Err(format!("{}: symlink in managed artifact path — refusing to follow", cursor.display()));
            }
            Ok(_) => {}
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => break,
            Err(e) => return Err(format!("{}: cannot inspect path component: {e}", cursor.display())),
        }
    }
    Ok(())
}
...
fn sanitize_file(project_path: &Path, path: &Path, boundary: &str) -> Result<usize, String> {
    ...
    if !meta.is_file() {
        return Err(format!("{}: managed audit artifact is not a regular file", path.display()));
    }

    let original = std::fs::read(path).map_err(|e| format!("{}: read failed: {e}", path.display()))?;
    let content = std::str::from_utf8(&original)
        .map_err(|_| format!("{}: managed audit artifact is not valid UTF-8", path.display()))?;
    ...
    crate::core::mcp_scanner::atomic_write_if_unchanged_preserving_permissions(path, &redacted, &original)
...
pub fn sanitize_files(project_path: &Path, files: &[PathBuf], boundary: &str) -> Result<Vec<(String, usize)>, String> {
...
mod tests {
    ...
    fn masks_literal_preserves_mode_and_leaves_clean_files_and_checksums() {
        ...
        write_mode(&td, "Committed `APP_SECRET=61cc954cdeadbeef0123456789abcdef` at .env.dist:7\n", 0o644);
        ...
        let targets = vec!["docs/AGENTS.md".to_string(), "docs/tech-debt/TD-app-secret.md".to_string()];
        ...
        assert!(!out.contains("61cc954cdeadbeef0123456789abcdef"), "value masked");
    ...
    fn marker_inside_secret_is_still_written_and_reported() {
        ...
        assert!(!out.contains(leak), "literal must not survive on disk: {out}");
        assert_eq!(out, "APP_SECRET=***REDACTED***\n");
        assert_eq!(report.len(), 1, "the changed artifact must be reported");
        assert!(report[0].1 > 0, "changed output must have positive telemetry");
    }
    ...
    fn refuses_symlinked_artifact_fail_closed() {
        ...
        assert!(err.contains("symlink"), "must refuse symlink fail-closed: {err}");
        // The link target was NOT followed/rewritten.
        assert!(std::fs::read_to_string(&real).unwrap().contains("Ab3xZ9Qw7Lm2Ns5Pt8Rv"));
    }
    ...
    fn missing_target_is_clean_not_an_error() {
        ...
        let report = sanitize_all(tmp.path(), &["docs/decisions.md".to_string(), "REVIEW".to_string()], "test").unwrap();
    ...
    fn refuses_targets_that_escape_project_scope() {
        ...
        assert!(err.contains("unsafe audit target outside project scope"), "{err}");
    }
    ...
    fn atomic_permission_helper_refuses_symlink_without_caller_precheck() {
        ...
        assert!(err.contains("symlink"), "helper must refuse directly: {err}");
    ...
    fn boundary_sweeps_every_managed_target_before_next_spawn() {
        ...
        write_mode(&proj.join("docs/repo-map.md"), "map with APP_SECRET=61cc954cdeadbeef0123456789abcdef here\n", 0o644);
        write_mode(&proj.join("docs/glossary.md"), "term: apikey=Ab3xZ9Qw7Lm2Ns5Pt8Rv\n", 0o644);
        ...
        assert!(!std::fs::read_to_string(proj.join("docs/repo-map.md")).unwrap().contains("61cc954cdeadbeef0123456789abcdef"));
        assert!(!std::fs::read_to_string(proj.join("docs/glossary.md")).unwrap().contains("Ab3xZ9Qw7Lm2Ns5Pt8Rv"));
        // Exactly the two secret-bearing files were reported; the clean one wasn't.
        assert_eq!(report.len(), 2, "both tainted managed targets redacted: {report:?}");
