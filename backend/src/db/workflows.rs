...
pub fn list_runs_page_complete_group(
    conn: &Connection,
    workflow_id: &str,
    minimum: u32,
    offset: u32,
    ...
    let Some(parent_run_id) = boundary.parent_run_id.clone() else {
        return Ok(runs);
    };
    let boundary_started_at = boundary.started_at;
    ...
    runs.extend(tail.into_iter().filter(|run| seen.insert(run.id.clone())));
...
/// Child sub-workflow rows for one parent, newest first.
///
/// Resume uses the persisted `trigger_context.__subwf_item__` cursor to match
/// a child back to its foreach item even when the item has no explicit `id`
/// (PR payloads commonly expose `number` instead).
pub fn subworkflow_children(conn: &Connection, parent_run_id: &str) -> Result<Vec<WorkflowRun>> {
    let sql = format!(
        "SELECT {} FROM workflow_runs
         WHERE parent_run_id = ?1 AND run_type = 'subworkflow'
         ORDER BY started_at DESC",
        WORKFLOW_RUN_COLS
    );
    let mut stmt = conn.prepare(&sql)?;
    let runs = stmt
        .query_map(params![parent_run_id], |row| Ok(row_to_run(row)))?
