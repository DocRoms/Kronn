...
#[test]
fn workflow_run_page_keeps_the_boundary_parent_group_complete() {
    use chrono::Duration;
    ...
    crate::db::workflows::insert_run(&conn, &sample_run("parent-run", "parent-wf")).unwrap();

    let base = Utc::now() - Duration::hours(1);
    for index in 0..5 {
        let mut older = sample_run(&format!("older-{index}"), "child-wf");
        older.started_at = base - Duration::seconds(index);
        crate::db::workflows::insert_run(&conn, &older).unwrap();
    }
    for index in 0..17 {
        let mut child = sample_run(&format!("child-{index}"), "child-wf");
        child.started_at = base + Duration::minutes(1) + Duration::seconds(index);
        child.parent_run_id = Some("parent-run".into());
        crate::db::workflows::insert_run(&conn, &child).unwrap();
    }

    let first =
        crate::db::workflows::list_runs_page_complete_group(&conn, "child-wf", 10, 0).unwrap();
    assert_eq!(
        first.len(),
        17,
        "the initial 10-run page must include all 17 children from the latest invocation"
    );
    assert!(first
        .iter()
        .all(|run| run.parent_run_id.as_deref() == Some("parent-run")));
