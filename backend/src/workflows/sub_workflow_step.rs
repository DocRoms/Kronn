...
fn foreach_item_id(item: &serde_json::Value, index: usize) -> String {
    for key in ["id", "number", "key"] {
...
async fn execute_foreach(
    ...
    let prior_children = {
        ...
        state
            .db
            .with_conn(move |c| crate::db::workflows::subworkflow_children(c, &pid))
            .await
            .unwrap_or_else(|e| {
                tracing::warn!(target: "kronn::sub_workflow", parent_run=%parent_run_id, error=%e,
                    "cannot load child rows — reconciliation falls back to the git ledger only");
                Default::default()
            })
    };
    let mut child_done_ids = std::collections::HashSet::new();
    let mut legacy_child_done_indexes = std::collections::HashSet::new();
    let mut interrupted_children_by_id = std::collections::HashMap::new();
    let mut legacy_interrupted_children_by_index = std::collections::HashMap::new();
    for child in prior_children
        .into_iter()
        .filter(|child| child.workflow_id == child_wf.id)
    {
        let item_index = child
            .trigger_context
            .as_ref()
            .and_then(|ctx| ctx.get("__subwf_item__"))
            .and_then(serde_json::Value::as_u64)
            .map(|index| index as usize);
        let item_id = child
            .trigger_context
            .as_ref()
            .and_then(|ctx| ctx.get("__subwf_item_id__"))
        ...
        match child.status {
            RunStatus::Success => {
                if let Some(id) = item_id {
                    child_done_ids.insert(id);
                } else if let Some(index) = item_index {
                    legacy_child_done_indexes.insert(index);
                }
            }
            RunStatus::Interrupted => {
                if let Some(id) = item_id {
                    interrupted_children_by_id.entry(id).or_insert(child);
                } else if let Some(index) = item_index {
                    legacy_interrupted_children_by_index
                        .entry(index)
                        .or_insert(child);
                }
            }
    ...
    for (idx, item) in items.iter().enumerate() {
        ...
        let item_id = foreach_item_id(item, idx);
        ...
        if !item_id.is_empty() {
            let git_done = crate::core::cmd::async_cmd("git")
                ...
                .unwrap_or(false);
            if git_done
                || child_done_ids.contains(&item_id)
                || legacy_child_done_indexes.contains(&idx)
            {
        ...
        tracing::info!(target: "kronn::sub_workflow", parent_run=%parent_run_id, child_run=%child_run.id, item=%idx, item_id=%item_id, resumed,
            "foreach: entering child");
...
mod tests {
        ...
        let child_wf = crate::models::Workflow {
            ...
            steps: vec![
                step_json(serde_json::json!({
                    "name": "prepare", "step_type": {"type": "JsonData"},
                    "json_data_payload": { "phase": "prepared" },
                })),
                step_json(serde_json::json!({
                    "name": "finish", "step_type": {"type": "JsonData"},
    ...
    #[tokio::test]
    async fn foreach_resume_uses_item_index_and_continues_the_interrupted_child() {
        let (state, tokens, agents, ws, step) = foreach_fixture().await;
        std::fs::write(
            ws.path().join("tasks.json"),
            r#"[{"number":1871},{"number":1872}]"#,
        )
        .unwrap();
            ...
            .with_conn(|c| {
                c.execute(
                    "INSERT INTO workflow_runs
                     ...
                     VALUES
                     ('done-child', 'child-wf', 'Success', 'subworkflow', 'parent-run',
                      '{\"__subwf_item__\":0,\"__subwf_item_id__\":\"\"}', '[]',
                ...
                c.execute(
                    "INSERT INTO workflow_runs
                     (id, workflow_id, status, run_type, parent_run_id,
                      trigger_context, step_results_json, started_at, finished_at)
                     VALUES
                     ('interrupted-child', 'child-wf', 'Interrupted', 'subworkflow', 'parent-run',
                      '{\"__subwf_item__\":1,\"__subwf_item_id__\":\"\"}',
                      '[{\"step_name\":\"prepare\",\"status\":\"Success\",\"output\":\"done\",\"tokens_used\":0,\"duration_ms\":1}]',
                      datetime('now'), datetime('now'))",
                    [],
                )?;
                Ok(())
        ...
        let data = run_foreach(&state, &tokens, &agents, &ws, &step).await;
        let items = data["items"].as_array().unwrap();
        assert_eq!(items[0]["status"], "SkippedAlreadyDone");
        assert_eq!(items[0]["id"], "1871");
        assert_eq!(items[1]["status"], "Success");
        assert_eq!(items[1]["child_run_id"], "interrupted-child");

        let children = state
            .db
            .with_conn(|c| crate::db::workflows::subworkflow_children(c, "parent-run"))
            .await
            .unwrap();
        assert_eq!(
            children.len(),
            2,
            "resume must not create duplicate children"
        );
        let resumed = children
            .iter()
            .find(|child| child.id == "interrupted-child")
            .unwrap();
        assert_eq!(resumed.status, crate::models::RunStatus::Success);
        assert_eq!(
            resumed
                .step_results
                .iter()
                ...
                .collect::<Vec<_>>(),
            vec!["prepare", "finish"],
            "the interrupted child continues after its persisted step"
        );
        assert!(
            resumed
                .state
                .contains_key(crate::workflows::runner::RUN_RESUME_HISTORY_KEY),
            "the child keeps a durable resume trail for the UI"
        );
    }
