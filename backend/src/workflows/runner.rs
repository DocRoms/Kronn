...
fn append_resume_transition(run: &mut WorkflowRun) {
    ...
    let mut history = run
        ...
        .and_then(|raw| serde_json::from_str::<serde_json::Value>(raw).ok())
        .unwrap_or_else(|| serde_json::json!({ "v": 1, "events": [] }));
    if history
        .get("events")
        .and_then(serde_json::Value::as_array)
        .is_none()
    {
        history = serde_json::json!({ "v": 1, "events": [] });
    }
    ...
    run.state
        .insert(RUN_RESUME_HISTORY_KEY.to_string(), history.to_string());
}

pub(crate) async fn claim_interrupted_run_row(
    state: &AppState,
    run: &mut WorkflowRun,
) -> Result<()> {
    use anyhow::anyhow;
...
mod tests {
    ...
    async fn resume_continues_from_the_step_after_the_last_completed_one() {
        ...
        let resume_history: serde_json::Value = serde_json::from_str(
            run.state
                .get(RUN_RESUME_HISTORY_KEY)
                .expect("resume history persisted"),
        )
        .unwrap();
        assert_eq!(
            resume_history["events"]
                .as_array()
                .unwrap()
                .iter()
                .filter_map(|event| event["status"].as_str())
                .collect::<Vec<_>>(),
            vec!["Interrupted", "Running"],
            "the UI can retain the interruption after the terminal status replaces Running"
        );
