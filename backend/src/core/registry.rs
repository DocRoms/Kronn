...
pub fn builtin_registry() -> Vec<McpDefinition> {
    vec![
        ...
        McpDefinition {
            id: "api-microsoft-365".into(),
            name: "Microsoft 365".into(),
            description: "Outlook (mail, calendar), Teams, OneDrive — Microsoft Graph via the Azure CLI. CLI/API only: the token is resolved in memory and never stored or written to a generated MCP config.".into(),
            transport: McpTransport::ApiOnly,
            env_keys: vec![],
            tags: vec![
                "api".into(),
            ...
            token_url: Some("https://aka.ms/installazurecli".into()),
            token_help: Some("Install the Azure CLI, then run `az login` once — Kronn reuses that local credential in memory for Graph API calls and never stores a token.\n\nInstall (cross-platform):\n- macOS:   brew install azure-cli\n- Windows: winget install Microsoft.AzureCLI\n- Linux:   curl -sL https://aka.ms/InstallAzureCLIDeb | sudo bash  (Debian/Ubuntu)\n- Any OS:  https://aka.ms/installazurecli\n\nThen `az login` (uses your org SSO/broker — passes Conditional Access where a raw device-code flow is blocked). Verify with `az account get-access-token --resource https://graph.microsoft.com`.".into()),
            publisher: "Microsoft (Azure CLI)".into(),
            official: true,
            alt_packages: vec![],
            default_context: Some("# Microsoft 365 — Usage Context\n\nDeterministic Microsoft Graph through the Azure CLI credential. No MCP server, no stored token: Kronn runs `az account get-access-token --resource https://graph.microsoft.com` in memory per request.\n\nBase: https://graph.microsoft.com/v1.0 . Prefer $select/$top to keep results small.\n- Unread inbox: GET /me/mailFolders/inbox/messages?$filter=isRead eq false&$select=subject,from,receivedDateTime&$top=15\n- Today agenda: GET /me/calendarView?startDateTime=...&endDateTime=...&$select=subject,start,end,location&$top=25\n- Send mail:    POST /me/sendMail (confirm first)\n- Teams post:   POST /chats/{chat_id}/messages (confirm first)\n\nIf readiness reports no identity, run `az login` on the host and retry. Never echo tokens.".into()),
            api_spec: Some(ApiSpec {
                ...
                auth: ApiAuthKind::CliToken {
                    ...
                    fallback_env_key: None,
                },
                docs_url: Some("https://learn.microsoft.com/graph/api/overview".into()),
                config_keys: vec![],
                endpoints: vec![
                    ApiEndpoint { path: "/me".into(), method: "GET".into(), description: "[USER · sanity] Signed-in user. Call first to verify the Azure CLI credential resolves a Graph token.".into() },
                    ApiEndpoint { path: "/me/messages".into(), method: "GET".into(), description: "[MAIL] Messages in the signed-in mailbox. Use $select + $top; $filter=isRead eq false for unread.".into() },
                    ApiEndpoint { path: "/me/mailFolders/inbox/messages".into(), method: "GET".into(), description: "[MAIL] Inbox messages only.".into() },
                    ApiEndpoint { path: "/me/calendarView".into(), method: "GET".into(), description: "[CALENDAR] Events in a window (startDateTime + endDateTime query params required).".into() },
                    ApiEndpoint { path: "/me/events".into(), method: "GET".into(), description: "[CALENDAR] Calendar events.".into() },
                    ApiEndpoint { path: "/me/sendMail".into(), method: "POST".into(), description: "[MAIL · write] Send a message as the signed-in user. Confirm before use.".into() },
