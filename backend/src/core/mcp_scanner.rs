...
"#;
    ...
    #[test]
    #[serial]
    fn project_and_host_sync_stay_in_owned_fixtures_and_restore_environment() {
        let root = tempfile::tempdir().unwrap();
        let writable_root = root.path().join("repos");
        let writable_project = writable_root.join("project");
        let host_fixture = root.path().join("host-fixture");
        let host_sentinel = root.path().join("host-sentinel");
        std::fs::create_dir_all(&writable_project).unwrap();
        std::fs::create_dir_all(&host_fixture).unwrap();
        std::fs::create_dir_all(host_sentinel.join(".codex")).unwrap();
        let sentinel_config = host_sentinel.join(".codex/config.toml");
        std::fs::write(&sentinel_config, "sentinel = 'must-not-change'\n").unwrap();
        ...
        std::env::set_var("HOME", &host_sentinel);
        std::env::set_var("USERPROFILE", &host_sentinel);
        std::env::set_var("KRONN_HOST_HOME", &host_fixture);
        ...
        crate::db::mcps::upsert_server(
            &writable_db,
            &crate::models::McpServer {
                id: "fixture-server".into(),
                name: "Fixture server".into(),
                description: String::new(),
                transport: crate::models::McpTransport::Stdio {
                    command: "echo".into(),
                    args: vec!["fixture-mcp".into()],
                },
                source: crate::models::McpSource::Registry,
                api_spec: None,
            },
        ...
        .unwrap();
        let fixture_env =
            HashMap::from([("MCP_TEST_TOKEN".to_string(), "fixture-secret".to_string())]);
        crate::db::mcps::insert_config(
            &writable_db,
            &crate::models::McpConfig {
                id: "fixture-config".into(),
                server_id: "fixture-server".into(),
                label: "fixture-mcp".into(),
                env_keys: vec!["MCP_TEST_TOKEN".into()],
                env_encrypted: crate::db::mcps::encrypt_env(&fixture_env, &secret).unwrap(),
                args_override: None,
                is_global: true,
                include_general: true,
                config_hash: "fixture-hash".into(),
                project_ids: vec![],
        ...
        sync_project_with_report(&writable_db, "sync-project", &secret);
        ...
        sync_project_with_report(&read_only_db, "sync-project", &secret);
        ...
        // This exercises the four real host-sync adapters. The sentinel is a
        // plausible fallback home, but the explicit fixture must win.
        sync_affected_projects(&writable_db, &["sync-project".to_string()], &secret);
        let codex_config = host_fixture.join(".codex/config.toml");
        ...
        // An unwritable fixture must fail closed instead of falling through to
        // HOME/USERPROFILE, including when the host adapters are invoked.
        let blocked_host_home = root.path().join("blocked-host-home");
        std::fs::write(&blocked_host_home, "not a directory").unwrap();
        std::env::set_var("KRONN_HOST_HOME", &blocked_host_home);
        sync_affected_projects(&writable_db, &["sync-project".to_string()], &secret);
        assert_eq!(
            std::fs::read_to_string(&sentinel_config).unwrap(),
            "sentinel = 'must-not-change'\n"
        ...
        // Concurrent adapters share the same owned target and must leave a
        // parseable config; the atomic writer may reject one stale writer.
        std::thread::scope(|scope| {
            for _ in 0..2 {
                scope.spawn(|| {
                    let conn = rusqlite::Connection::open_in_memory().unwrap();
                    crate::db::migrations::run(&conn).unwrap();
                    sync_affected_projects(&conn, &[], &secret);
                });
            }
        });
        toml::from_str::<toml::Table>(&std::fs::read_to_string(&codex_config).unwrap()).unwrap();
        assert_eq!(
            std::fs::read_to_string(&sentinel_config).unwrap(),
            "sentinel = 'must-not-change'\n"
        );

        let expected_host_home = restore
            .values
            .iter()
            .find(|(name, _)| *name == "KRONN_HOST_HOME")
            .unwrap()
            .1
            .clone();
        drop(restore);
        assert_eq!(std::env::var_os("KRONN_HOST_HOME"), expected_host_home);
    }
}
