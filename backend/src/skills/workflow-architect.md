...
> **Batch output is directly pipeable after completion.** With `batch_wait_for_completion: true`, `{{steps.<name>.data.results}}` is ordered like the input and contains `{ index, discussion_id, item, output, tokens_used, tokens_status, dispatch }` for every child. `dispatch.attempts` and `dispatch.redispatches` expose paid reruns; the aggregate repeats them as `dispatch_attempts` / `redispatches`. `output` is the complete final Agent message, without transport truncation, so a downstream `Exec`/`ApiCall` can parse all results without an Agent collector. Aggregate `tokens_used` is accompanied by `tokens_status`: `measured`, `partial`, or `unavailable_children_not_measured`; per child it is `measured` or `not_measured` — a missing measure is never disguised as a real zero. Fire-and-forget mode returns `results: []` and `tokens_status: pending`; it is refused when the parent owns an isolated worktree, because cleanup cannot race child readers.

> **Evidence must be unforgeable.** Never validate a pack or artifact only by comparing a model-reported value (digest, HEAD, path or status) with another value supplied in its prompt: the model can repeat the expected value without having read the source. Enforce filesystem/API preconditions before dispatch or verify the artifact mechanically afterward. Batch items carrying `repo_path` fail before any child is created when that directory is absent or is not a Git worktree (`PACK PRECONDITION FAIL`).

> **Do NOT chain an `Agent` step directly behind a batch.** Because `output` carries every child's full message, `{{steps.<name>.data.results}}` reaches hundreds of kilobytes on a 20-item fan-out — all of it injected verbatim into the next prompt. Pipe a batch into `Exec` or `ApiCall`, which parse without paying for context; if a judgement call really has to follow, insert an `Exec` that reduces the payload first and feed the Agent only what it must read.
...
4. **Is the destination a readable, automatically refreshed HTML report?** → `PublishPageData`. Call `page_list()` first; reuse a Page or create it with `page_create()` before the workflow. If the Page exists but lacks the named dataset, declare it explicitly with `page_add_dataset()` before authoring the write. Add `TransformData` when the Page should consume a stable business contract, or publish the collector envelope directly when lossless raw data is intentional.
...
- `page_list()` — every shared Live Page. Required before any `PublishPageData` step. Reuse a matching id; if none fits and the user authorized autonomous creation, call `page_create()` first and use its returned id. For an existing Page whose contract lacks a required dataset, call `page_add_dataset()` with its stable name and kind instead of recreating the Page.
