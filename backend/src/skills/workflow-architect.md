...
> **Batch output is directly pipeable after completion.** With `batch_wait_for_completion: true`, `{{steps.<name>.data.results}}` is ordered like the input and contains `{ index, discussion_id, item, output, tokens_used, tokens_status }` for every child. `output` is the complete final Agent message, without transport truncation, so a downstream `Exec`/`ApiCall` can parse all results without an Agent collector. Aggregate `tokens_used` is accompanied by `tokens_status`: `measured`, `partial`, or `unavailable_children_not_measured`; per child it is `measured` or `not_measured` — a missing measure is never disguised as a real zero.  Fire-and-forget mode returns `results: []` and `tokens_status: pending` because the children have not finished yet.
