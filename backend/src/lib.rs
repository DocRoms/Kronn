...
pub struct AppState {
    ...
    /// Dependency update checks may hit remote registries, so project
    /// overviews reuse results until a manifest changes or the cache expires.
    pub dependency_update_cache: Arc<
        tokio::sync::Mutex<
            HashMap<String, crate::core::dependency_updates::CachedDependencyUpdates>,
        >,
    >,
    /// Cached project language breakdowns. Git status itself stays live; only
...
impl CancelGuard {
    /// Atomically reserve `key` only when no run is already registered.
    ///
    /// Peer-agent messages can arrive concurrently. Their native-run
    /// dispatcher uses this variant so two nearly-simultaneous replies do
    /// not start two local agents for the same discussion.
    pub fn try_insert(
        registry: &Arc<Mutex<HashMap<String, tokio_util::sync::CancellationToken>>>,
...
fn is_destructive(method: &axum::http::Method, path: &str) -> bool {
    method == axum::http::Method::DELETE
        ...
        // A page-bindings WRITE (POST upsert / PUT) DEFINES the authorization
        // policy the decide/gate/trigger endpoints above enforce — workflow_id,
        // allowed_gate_steps, trigger_variable_allowlist. Under the auth-off
        // default a LAN peer must not be able to rewrite that policy, so the
        // write is destructive. Scoped by METHOD so the GET list stays open (a
        // read), and by suffix so `/api/pages/{id}/bindings/{dataset}` DELETE is
        // left to the `method == DELETE` arm above.
...
pub fn build_router_with_auth(state: AppState, enable_auth: bool) -> Router {
    ...
    let mut router = Router::new()
        ...
        .route(
            "/api/projects/{id}/source-files",
            get(api::ai_docs::list_source_files),
        )
        .route(
            "/api/projects/{id}/source-file",
            get(api::ai_docs::read_source_file),
        )
        .route(
            "/api/projects/{id}/source-search",
            get(api::ai_docs::search_source_files),
        )
        .route(
            "/api/projects/{id}/source-exclusions",
            get(api::ai_docs::get_source_exclusions).put(api::ai_docs::set_source_exclusions),
        )
        .route(
        ...
        // 0.10.0 — Continual Learning (spec docs/research/continual-learning-0.10.0-spec.md)
        .route(
        ...
        .route(
            "/api/workflows/{id}/runs/count",
            get(api::workflows::count_runs),
        )
        .route(
...
mod cancel_guard_tests {
    ...
    #[test]
    fn try_insert_reserves_one_run_per_key_until_guard_drops() {
        let registry = Arc::new(Mutex::new(HashMap::new()));
        let first = CancelGuard::try_insert(&registry, "disc-1").expect("first run claims key");

        assert!(
            CancelGuard::try_insert(&registry, "disc-1").is_none(),
            "a concurrent run for the same discussion is rejected"
        );
        assert!(
            CancelGuard::try_insert(&registry, "disc-2").is_some(),
            "another discussion remains independent"
        );

        drop(first);
        assert!(
            CancelGuard::try_insert(&registry, "disc-1").is_some(),
            "the key becomes available when its run ends"
        );
    }
}
...
mod auth_tests {
    ...
    fn destructive_posts_inventory_is_gated() {
        ...
        for p in [
            ...
            // A page-bindings WRITE defines the auth policy the decide/trigger
            // endpoints enforce — a remote no-token caller must not rewrite it.
            "/api/pages/my-page/bindings",
        ] {
            assert!(
                !auth_allows(&Method::POST, p, false, true, false, false),
                "{p} must deny remote+no-token"
    ...
    #[test]
    fn page_bindings_write_is_destructive_but_read_is_open() {
        const BINDINGS: &str = "/api/pages/my-page/bindings";
        // WRITE (POST upsert / PUT) rewrites the authorization policy → gated
        // even under the auth-off default; open only to local or a valid token.
        for method in [Method::POST, Method::PUT] {
            assert!(
                !auth_allows(&method, BINDINGS, false, true, false, false),
                "{method} /bindings must deny remote+no-token"
            );
            assert!(
                auth_allows(&method, BINDINGS, false, true, true, false),
                "{method} /bindings allows local"
            );
        }
        // The GET list is a read — it keeps the historical auth-off-is-open rule.
        assert!(
            auth_allows(&Method::GET, BINDINGS, false, true, false, false),
            "GET /bindings must stay open (read)"
        );
        // DELETE unbind is covered by the verb arm, not the /bindings suffix, so
        // the parametrized `/bindings/{dataset}` path is still gated.
        assert!(
            !auth_allows(
                &Method::DELETE,
                "/api/pages/my-page/bindings/pipeline",
                false,
                true,
                false,
                false
            ),
            "DELETE unbind must deny remote+no-token"
        );
    }
