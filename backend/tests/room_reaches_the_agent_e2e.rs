...
fn isolate_config_dir() {
    ...
    let root = FIXTURE_ROOT
        .get_or_init(|| tempfile::tempdir().expect("create room-reaches-agent fixture root"));
    let data_dir = root.path().join("data");
    let host_home = root.path().join("host-home");
    std::fs::create_dir_all(&data_dir).expect("create room-reaches-agent data fixture");
    std::fs::create_dir_all(&host_home).expect("create room-reaches-agent host fixture");
